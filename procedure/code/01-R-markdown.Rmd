...
# if you load dplyr and use dplyr::filter(), R has to guess whether you mean dplyr::filter() or stats::dplyr::filter()
...
original_gee %>%
  ...
  dplyr::filter(or_variable != "Incidence") %>%
...
original_gee %>%
  dplyr::filter(recalc_Incidence != Incidence) %>%
...
acs_covid %>%
  st_drop_geometry %>%
  dplyr::filter(covid_rate != or_incidence) %>%
  arrange(fips) %>%
...
tm_spatialepi_clusters <-
  ...
  tm_shape(acs_covid) +
    tm_polygons(
      fill = "isCluster",
      fill.scale = tm_scale(values = "oranges", value.na = "white"),
...
cat(
  ...
  dplyr::filter(REL_RISK == or_rel_risk & REL_RISK > 0) %>%
  nrow(),
...
gini_circle <- satscan_rpr_col %>% dplyr::filter(GINI_CLUST == 'T')
hier_circle <- satscan_rpr_col %>% dplyr::filter(GINI_CLUST == 'F')
...
satscan_rpr_gini <- satscan_rpr_gis_t %>% dplyr::filter(GINI_CLUST == "T") %>% select(fips, gini_rr = CLU_RR)
satscan_rpr_hier <- satscan_rpr_gis_t %>% dplyr::filter(GINI_CLUST == "F") %>% select(fips, hier_rr = CLU_RR)
...
gee_data <- acs_covid %>%
# dplyr::filter(covid_rate > 0) %>% # moved filtering to after z-score calculation
  mutate(
  ...
  dplyr::filter(covid_rate > 0) # moved filtering from before z-score calculation
```
